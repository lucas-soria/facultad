...
def plot_family(index, ax1, ax2, reindex = 1):
    for i in range(len(familia_curvas)):
        wind, anglexz, angley = familia_curvas[i]
        z = h + vi * np.sin(alpha) * (index * tick) - 1/2 * g * (index * tick)**2
        if z < 0:
            xlw[i].append(vi * np.cos(alpha) * (ttotal))
            ylw[i].append(vi * np.sin(teta) * (ttotal))
            zlw[i].append(h + vi * np.sin(alpha) * (index * tick) - 1/2 * g * (ttotal)**2)
            continue
        else:
            x = vi * np.cos(alpha) * (index * tick)
            y = vi * np.sin(teta) * (index * tick)
            if z >= h23:
                x += wind * np.cos(anglexz) * (reindex * tick) 
                y += wind * np.sin(angley) * (reindex * tick)
                reindex += 1
            if i in xlw:
                xlw[i].append(x)
                ylw[i].append(y)
